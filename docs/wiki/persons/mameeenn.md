...
const nu_nValue = ref("");
const nu_nRunBtn = ref("Run");
let intervalId = -1;
function nu_n(){
  if(intervalId !== -1){
    clearInterval(intervalId);
    intervalId = -1;
    nu_nRunBtn.value = "Run";
  } else {
    let length = 0;
    const LIMIT = 40;
    let isDecrementing = false;
    intervalId = setInterval(()=>{
      length += isDecrementing ? -1 : 1;
      nu_nValue.value = `ぬ${"ー".repeat(length)}ん`
      if(length >= LIMIT || length <= 0){
        isDecrementing = !isDecrementing;
      }
    },50)
    nu_nRunBtn.value = "Stop";
  }
}
</script>
...
<button @click="nu_n">[ {{nu_nRunBtn}} ]</button>
